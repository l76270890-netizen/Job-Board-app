...
import {
  ...
  ChevronRight, X, Upload, Check, Camera, Building2, Loader
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useEffect, useState, useRef } from "react";
import { useAuth } from "../context/AuthContext";
import { db } from "../lib/firestoreCompat";
import { storage } from "../lib/storageCompat";
import { doc, updateDoc } from "../lib/firestoreCompat";
import { ref, uploadBytes, getDownloadURL } from "../lib/storageCompat";

export default function SettingPage() {
  const navigate = useNavigate();
  const { currentUser, logout, userData, changePassword } = useAuth();
  ...
  const handleChangePassword = async () => {
    if (!password.current || !password.new) return alert("Enter your current and new passwords.");
    if (password.new !== password.confirm) return alert("The new passwords do not match.");
    try {
      ...
      alert(error.message || "Could not change your password.");
    }
  };
